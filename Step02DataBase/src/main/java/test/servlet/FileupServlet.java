...
public class FileupServlet extends HttpServlet{
	...
	// post 방식 요청만 처리할거면 doPost() 메소드-> 오버라이드
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		/*
		 * "caption"이라는 파라미터명으로 문자열이 전송되고
		 * "myFile" 이라는 파라미터명으로 파일이 전송됨(추출 방식이 다름)
		 */	
		String caption=req.getParameter("caption");
		// <input type="file" name="myfile"> -- name과 일치하도록 getPart("name")
		Part filePart = req.getPart("myFile");
		
		//만일 업로드된 파일 이미지가 있다면 (수정하지 않았으면 없다)
		if(filePart!=null && filePart.getSize() > 0) {
			//원본 파일의 이름 얻어내기 -원본파일명
			...
			/*
			 * 원래는 DB에 저장해야하지만 테스트로 응답에 필요한 데이터를 
			 * HttpServletRequest 객체의 .setAttribute(키, 값) 메소드 이용해서 저장하고
			 * jsp 페이지가 사용할 수 있도록 한다.
			 * jsp 페이지에서 해당 정보를 얻어낼 때는
			 * HttpServletRequest 객체의 .getAttribute(키) 이용해서 얻어냄
			 * Object type으로 리턴되기 떄문에 원래 type으로 casting 해야함
			 */
		...
		// 요청 전달자 객체 얻어내기
		RequestDispatcher rd=req.getRequestDispatcher("/test/kkk.jsp");
		// 응답을 위임하기
		rd.forward(req, resp);

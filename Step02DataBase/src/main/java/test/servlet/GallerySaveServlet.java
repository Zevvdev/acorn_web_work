...
public class GallerySaveServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        // 파일이 여러 개라서 getParts() 메소드 호출->리턴되는 Part 목록 사용하기
        Collection<Part> parts=req.getParts();

        // 이미지 저장
        for (Part part : parts) {
			/*
			 * <input type="file" name="images">의 
			 * name 속성값이 images이기 때문에 
			 * if문에서 확인한다
			 * part.getName()메소드가 리턴하는 문자열 확인하면 어떤 input type="file"요소의 데이터인지 구별할 수 있음
			 */

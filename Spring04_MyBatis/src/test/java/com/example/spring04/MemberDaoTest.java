...
// Assertions 클래스에 존재하는 모든 static 자원을 import 해서
// 마치 이 클래스의 멤버 메소드처럼 사용할 수 있다.
...
@Rollback // insert, update, delete 등의 동작은 rollback 이 되도혹 한다.
public class MemberDaoTest {
	...
	//이름과 주소를 전달하면 전달된 정보를 DB에 저장하고 해당 정보의 PK를 리턴하는 메소드
	int insertFixture(String name, String addr) {
		MemberDto dto=new MemberDto();
		dto.setName(name);
		dto.setAddr(addr);
		//mybatis에서 selectkey 설정을 했기 때문에 insert 하고나면 dto객체에 추가한 회원의 번호가 들어있다
		dao.insert(dto);
		return dto.getNum();
	}
	...
	void selectAll2() {
		...
		//회원 목록을 select해서 목록에 방금 추가된 정보가 잘 들어있는지 확인한다.
		List<MemberDto> list=dao.selectAll();
		/*
		 * item.getNum()==num 이 하나라도 true가 있다면 (하나라도 일치한다면)
		 * true를 리턴하는 메소트가 .anyMatch() 메소드이다.
		 */
	...
	void delete() {
		int num=insertFixture("acorn", "seoul");
		//삭제하고 삭제된 row의 갯수를 리턴 받는다.
		int rows=dao.deleteByNum(num);
		//삭제된 row의 갯수는 반드시 1이어야 한다.
		assertEquals(1, rows);
	...
	void insert() {
		// 미리 만들어 둔 메소드로 데이터를 넣고 PK를 리턴 받아서 사용한다.
		int num=insertFixture("acorn", "seoul");
		//데이터가 잘 들어갔는지 select 해보기
		MemberDto found=dao.getByNum(num);
		//select 된 정보가 null이 아니어야 한다.
		assertNotNull(found);
		//select된 정보의 name이 반드시 "acorn"이어야 한다.
		assertEquals("acorn", found.getName());
		//select된 정보의 addr이 반드시 "seoul"이어야 한다.

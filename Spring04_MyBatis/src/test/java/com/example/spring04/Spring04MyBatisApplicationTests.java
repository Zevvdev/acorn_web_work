...
/*
 * 여기에 test 코드를 넣고 run as jUnit test를 하면 단위 테스트를 할 수 있다.
 */
@SpringBootTest
class Spring04MyBatisApplicationTests {
	...
	@Test
	@DisplayName("MemberDao 와 SqlSession 객체 DI 성공여부 테스트")
	void test01() {
		Assertions.assertNotNull(session, "SqlSession 은 null 이면 안됨");
		Assertions.assertNotNull(dao, "MemberDao 는 null 이면 안됨");
	}
	...
	void selectTest() {
		...
		Assertions.assertNotNull(list,"selectAll()은 null이 아니어야 한다");
